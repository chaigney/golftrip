...
function holeBgForPar(par) {
  // black text, white/gray backgrounds per hole by par
...
function colorForRelative(score, par) {
  // numbers colorize by score vs par
...
export default function App() {
  ...
  const addPlayer = (name = "") => {
    ...
    // Ensure score array exists for this course
    setScoresByCourse(prev => ({
      ...prev,
      [courseKey]: { ...(prev[courseKey] || {}), [p.id]: empty18() }
    }))
    scheduleSave()
  }
  ...
  const removePlayer = (id) => {
    ...
    // remove from any team slots
    setTeams(ts => ts.map(t => ({ ...t, playerIds: (t.playerIds || []).map(pid => pid === id ? "" : pid) })))
    // remove their score row on this course
    setScoresByCourse(prev => {
      const next = { ...prev }
      const m = { ...(next[courseKey] || {}) }
      delete m[id]
      next[courseKey] = m
      return next
    })
  ...
  const addTeam = () => {
    setTeams(ts => [...ts, { id: uid(), name: `Team ${ts.length + 1}`, playerIds: ["",""] }])
    scheduleSave()
  }
  ...
  const assignPlayer = (teamId, slotIdx, newPid) => {
    setTeams(ts => {
      if (!newPid) {
        // clear slot
        return ts.map(t => t.id === teamId ? {
          ...t, playerIds: Object.assign([], t.playerIds, { [slotIdx]: "" })
        } : t)
      }
      // remove from all teams first
      const cleared = ts.map(t => ({
        ...t,
        playerIds: (t.playerIds || []).map(pid => pid === newPid ? "" : pid)
      }))
      // put into requested slot
      return cleared.map(t => t.id === teamId ? {
        ...t, playerIds: Object.assign([], t.playerIds, { [slotIdx]: newPid })
      } : t)
    })
    scheduleSave()
  }
   ...
   * --------------------------------------------*/
  const addMatch = () => {
    setMatches(ms => [...ms, { id: uid(), teamAId: "", teamBId: "", mode: MODES[0].id }])
    scheduleSave()
  }
  const removeMatch = (mid) => { setMatches(ms => ms.filter(m => m.id !== mid)); scheduleSave() }
  const setMatchField = (mid, field, value) => {
    setMatches(ms => ms.map(m => m.id === mid ? { ...m, [field]: value } : m))
    scheduleSave()
  }
  ...
  const ensurePlayerScores = (pid) => setScoresByCourse(prev => {
    ...
    if (!m[pid]) {
      return { ...prev, [courseKey]: { ...m, [pid]: empty18() } }
    }
    return prev
  })
  ...
  const getScore = (pid, h) => {
    ...
    if (v === "" || v === null || v === undefined) return NaN // empty = not entered
    const n = Number(v)
  ...
  function computeHole(modeId, h, A, B) {
    ...
    // High–Low / 2 points (1 for each low and high)
    if (modeId === "highlow") {
      let aPts = 0, bPts = 0
    ...
    // Captain & Mate / 2 points (captain is first listed player automatically)
    ...
    // Stableford / 1 point for winning team (tie halves)
  ...
  // Skins carry: convert per-hole results into carry-aware totals
  function applySkinsCarry(perHole) {
    ...
    return perHole.map((r) => {
      ...
      if (r.aPts === 0 && r.bPts === 0) { // halved
        carry += 1
        return { ...r, aPts: 0, bPts: 0, info: `Skin halved${carry ? ` (carry ${carry})` : ""}` }
      }
      if (r.aPts > r.bPts) { // A wins, award carry+1
        const pts = carry + 1
        carry = 0
        return { ...r, aPts: pts, bPts: 0, info: `Skin A x${pts}` }
      }
      if (r.bPts > r.aPts) { // B wins, award carry+1
        const pts = carry + 1
        carry = 0
        return { ...r, aPts: 0, bPts: pts, info: `Skin B x${pts}` }
      }
      return r
    })
  }
  ...
  return (
    <div style={{ fontFamily: 'system-ui, sans-serif', color: '#000', padding: 16 }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
        ...
        <div>
          <select value={courseKey} onChange={e => { setCourseKey(e.target.value); scheduleSave() }}>
